  ...
  return (
    <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${color}`}>
      <span className={`w-1.5 h-1.5 rounded-full ${
        ...
        status === 'baixo_estoque' ? 'bg-amber-400' : 'bg-red-400'
      }`} />
      {label}
    </span>
  );
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <input
        className={`
          ...
          placeholder:text-[#8b91a8] text-sm
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <textarea
        ...
        className={`
          bg-[#2e3347] border border-[#2e3347] text-[#e8eaf0] rounded-xl px-4 py-2.5
          placeholder:text-[#8b91a8] text-sm resize-none
          focus:outline-none focus:border-[#4f6ef7] focus:ring-1 focus:ring-[#4f6ef7]/30
          transition-colors
...
export function Card({ children, className = '', ...props }) {
  return (
    <div
      className={`bg-[#1a1d27] border border-[#2e3347] rounded-2xl ${className}`}
      {...props}
    >
      {children}
    </div>
